...
get_bbg <- function(tickers, field = NULL, start_date, end_date = NULL, bbg_options = NULL, bbg_overrides = NULL, include_non_trading_days=FALSE, ...)
{
  # Ensure field is singular
  if (length(field) > 1) stop("get_bbg: does not support more than one field.")
  ...
  tryCatch(start_date <- as.Date(start_date), error = function(e) stop("get_bbg: start_date cannot be coerced into Date object."))
  tryCatch(end_date   <- as.Date(end_date), error = function(e) stop("get_bbg: end_date cannot be coerced into Date object."))
...
requests <- yaml::read_yaml(paste0(root, "/requests.yml"))[[1]]
